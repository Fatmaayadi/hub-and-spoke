...
locals {
  osn_cidrs = {
    "all-cdg-services-in-oracle-services-network" = "138.1.0.0/16"
  }
  #Retrieves OCI service endpoints (e.g., Object Storage, Autonomous Database) as a mapping of CIDR blocks to service IDs.
  subnets = flatten([  #This loops through var.vcns (which holds VCN details) and extracts subnet details.
    for k, v in var.vcns : [
  ...
  #Extracts security list configurations for each subnet
...
#Fetches all OCI services, used to configure Service Gateway.
...
resource "oci_core_vcn" "these" {
  ...
  dns_label      = replace(lower(each.key), "-", "")
...
resource "oci_core_internet_gateway" "these" {
  for_each       = { for k, v in var.vcns : k => v if v.is_create_igw == true } #Creates Internet Gateways (IGWs) only if is_create_igw is true.
    compartment_id = each.value.compartment_id
...
### Service Gateway    Allows access to OCI services (e.g., Object Storage) privately.
...
resource "oci_core_drg_attachment" "these" {
  for_each     = { for k, v in var.vcns : k => v if v.is_attach_drg == true } # only if true
    drg_id        = var.drg_id
    vcn_id        = oci_core_vcn.these[each.key].id
...
resource "oci_core_default_security_list" "these" {
  for_each = oci_core_vcn.these
    ...
    ingress_security_rules {
      ...
      stateless = false
      source    = "0.0.0.0/0"
      icmp_options { # allowing ICMP traffic by default  for diagnostics
        type = 3
        code = 4
      }
    }
...
# we can define a customized securrity list for a specific subnet
