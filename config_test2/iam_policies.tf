 ...
 policies_configuration = {
    ...
    supplied_policies = {
      policy1 = {
        ...
        description = "Full access to prod compartment"
        compartment_ocid = local.enclosing_compartment_id
        statements  = [
          "Allow group Administrators to manage all-resources in compartment prod"
        ]
      }
      policy2 = {
        name        = "test-read-access"
        description = "Read-only access to test"
        compartment_ocid = local.enclosing_compartment_id
        statements  = [
          "Allow group Developers to inspect all-resources in compartment test"
        ]
      }
    }
  }
  ...
  # Configuration si enable_access_governance_policies == true
  access_governance_policies_configuration = {
    policy1 = {
      name        = "ag-access"
      description = "Access governance policy for security"
      statements  = [
        "Allow group SecurityAdmins to read users in tenancy"
      ]
    }
  }

  # Cas par défaut (vide) si les policies ne sont pas activées
  #-------------------------------------------------------------------------- 
  ...
  security_admin_policy_name         = "${var.service_label}-security-admins-policy"
  security_admin_root_policy_name    = "${var.service_label}-security-admins-root-policy"
  network_admin_policy_name          = "${var.service_label}-network-admins-policy"
  network_admin_root_policy_name     = "${var.service_label}-network-admins-root-policy"
  compute_agent_policy_name          = "${var.service_label}-compute-agent-policy"
  database_admin_policy_name         = "${var.service_label}-test-admins-policy"
  database_dynamic_group_policy_name = "${var.service_label}-test-dynamic_group-policy"
  appdev_admin_policy_name           = "${var.service_label}-prod-admins-policy"
  ...
  //access_governance_policy_name      = "${var.service_label}-access-governance-policy"
...
## Security admin grants on Security compartment
  security_admin_grants_for_oag_on_security_cmp = var.enable_access_governance_policies == true ? [
        # Permissions for Security Admin to create and manage OAG instances https://docs.oracle.com/en/cloud/paas/access-governance/cagsi/#GUID-CC633334-77F1-46A9-AAD3-5B1210BAE2EE
  ...
  local.security_admin_grants_on_appdev_cmp, local.security_admin_grants_on_database_cmp, local.security_admin_grants_for_oag_on_security_cmp)
...
*/
  ...
  /*access_governance_policies = {
    (local.access_governance_policy_name) = {
      compartment_ocid = var.tenancy_ocid
      name             = local.access_governance_policy_name
      description      = "CIS Landing Zone Access Governance at root compartment policy."
      defined_tags     = local.policies_defined_tags
      freeform_tags    = local.policies_freeform_tags
      statements = [
        "allow group ${join(",", local.access_governance_group_name)} to inspect all-resources in tenancy",
        "allow group ${join(",", local.access_governance_group_name)} to read policies in tenancy",
        "allow group ${join(",", local.access_governance_group_name)} to read domains in tenancy",
        # Permissions for Security Admin to create and manage OAG instances https://docs.oracle.com/en/cloud/paas/access-governance/cagsi/#GUID-CC633334-77F1-46A9-AAD3-5B1210BAE2EE
        "allow group ${join(",", local.security_admin_group_name)} to read objectstorage-namespace in tenancy",
      ]
    }
  }*/
}
...
module "lz_access_governance_policies" {
  depends_on             = [module.lz_compartments, module.lz_groups, module.lz_dynamic_groups]
  source                 = "github.com/oracle-quickstart/terraform-oci-cis-landing-zone-iam//policies?ref=v0.1.7"
  providers              = { oci = oci.home }
  tenancy_ocid           = var.tenancy_ocid
  //policies_configuration = var.extend_landing_zone_to_new_region == false && var.enable_access_governance_policies ? local.access_governance_policies_configuration : local.empty_policies_configuration
...
locals {
  ...
  /*access_governance_policies_configuration = {
    enable_cis_benchmark_checks : true
    supplied_policies : local.access_governance_policies
  }*/
...
resource "oci_identity_policy" "cloud_guard_policy" {
  name           = "${var.service_label}-cloud-guard-policy"
  description    = "Allow Security Admins to manage Cloud Guard"
  compartment_id = var.tenancy_ocid

  statements = [
    "Allow group ${local.provided_security_admin_group_name} to manage cloud-guard-family in tenancy"
  ]
}
