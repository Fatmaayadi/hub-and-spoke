...
locals {
  ...
  valid_service_gateway_cidrs = [
   ...
   network_entity_ids = {
    ...
    subnet_name => lookup(module.lz_vcn_spokes.service_gateways, subnet.vcn_id, null) != null ? lookup(module.lz_vcn_spokes.service_gateways, subnet.vcn_id, null).id : null
  }

  spoke_subnet_names_raw = length(var.subnets_names) == 0 ? ["web", "app", "db"] : var.subnets_names
  spoke_subnet_names_per_vcn = [for s in local.spoke_subnet_names_raw : split(",", s)]
  ...
  auto_vcns_map = {
  for idx, v in var.vcn_cidrs :
    "vcn${idx}" => {
      name         = length(var.vcn_names) > 0 ? (
                       length(regexall("[a-zA-Z0-9-]+", var.vcn_names[idx])) > 0
                       ? join("", regexall("[a-zA-Z0-9-]+", var.vcn_names[idx]))
                       : var.vcn_names[idx]
                     ) : "${var.service_label}-${idx}-vcn"
      cidr         = v
      subnet_names = local.spoke_subnet_names_per_vcn[idx]
      subnet_cidrs = cidrsubnets(v, local.spoke_subnet_size[idx]...)
    }
}
...
#      cidr            = cidrsubnet(vcn.cidr, 4, index(local.spoke_subnet_names, s))
      ...
      dns_label       = s
      private         = length(var.dmz_vcn_cidr) > 0 || var.no_internet_access ? true : (index(vcn.subnet_names, s) == 0 ? false : true)
  ...
  web_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("${key}-rtable", "vcn-", "") => {
    ...
    defined_tags   = local.vcn_defined_tags
    freeform_tags  = local.vcn_freeform_tags
    route_rules = [for rule in concat([{
      ...
      network_entity_id = lookup(module.lz_vcn_spokes.service_gateways, subnet.vcn_id, null) != null ? lookup(module.lz_vcn_spokes.service_gateways, subnet.vcn_id, null).id : null
      ...
      },
      {
        ...
        destination       = local.anywhere
        destination_type  = "CIDR_BLOCK"
        network_entity_id = length(var.dmz_vcn_cidr) == 0 && ! var.no_internet_access ? (
          lookup(module.lz_vcn_spokes.internet_gateways, subnet.vcn_id, null) != null ? lookup(module.lz_vcn_spokes.internet_gateways, subnet.vcn_id, null).id : null
        ) : null
        description       = "Traffic destined to ${local.anywhere} goes to Internet Gateway."
      }
      ],
    ...
    ) : rule if rule.is_create && rule.network_entity_id != null]
  } if length(regexall(".*-${local.spoke_subnet_names_per_vcn[0][0]}-*", key)) > 0 }
  ...
  backend_route_tables = { for key, subnet in local.all_lz_spoke_subnets : replace("${key}-rtable", "vcn-", "") => {
    ...
    route_rules = [for rule in concat([{
      ...
      },
      {
        ...
        destination_type  = "CIDR_BLOCK"
        network_entity_id = length(var.dmz_vcn_cidr) == 0 && ! var.no_internet_access ? (
          lookup(module.lz_vcn_spokes.nat_gateways, subnet.vcn_id, null) != null ? lookup(module.lz_vcn_spokes.nat_gateways, subnet.vcn_id, null).id : null
        ) : null
...
module "lz_route_tables_spokes" {
  ...
  subnets_route_tables = local.lz_subnets_route_tables
   internet_gateway_id = {
    for subnet_key, subnet_val in local.lz_subnets_route_tables :
    subnet_key => lookup(module.lz_vcn_spokes.internet_gateway_id, subnet_val.vcn_id, null)
  }
} 
/*module "vcn_basic" {
  source               = "../modules/network/vcn-basic"
